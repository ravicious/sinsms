...
class Brameczka
  ...
  class<<self
    def sms(params)
      ...
      resp = http.get('/images/spacer.gif', HEADERS.merge({ 'Cookie' => cookie }))
      ...
      if data =~ /<font size="\+2">(.*?)<\/font>/
        # puts "Status: #{$1}"
        return_hash[:status] = $1.encode("UTF-8", :invalid => :replace, :undef => :replace, :replace => "?")

      end

      if data =~ /<center>(Zosta.*?)<\/center>/
        # puts $1
        return_hash[:count] = $1.encode("UTF-8", :invalid => :replace, :undef => :replace, :replace => "?")
